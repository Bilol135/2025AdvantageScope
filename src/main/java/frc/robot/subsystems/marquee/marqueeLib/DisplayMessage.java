...
/**
 * A message to send to the display controller. The message is an array of bytes containing a
 * pipe-delimited command string having the following fields:
 *
 * <pre>
 *
 * | Field No. | Contents                                           |
 ...
 * </pre>
 *
 * Commands MUST end with a new-line (\n'). For Windows compatibility, a carriage return ('\r') MAY
 * appear before the terminating new-line.
 *
 * <p>Display commands are specified by the DisplayCommand enum defined in DisplayCommand.java.
 *
 * <p>Intensity values must be non-negative and less than or equal to 255.
 *
 * <p>Delays must be non-negative.
 *
 * <p>Note that delay semantics vary among commands, and may be ignored entirely.
 *
 * @see DisplayCommand
 */
public final class DisplayMessage {
  ...
  private static String ERROR_STRING =
      new DisplayMessage().setDisplayCommand(DisplayCommand.ERROR).toString();
  ...
  private int checkColor(int value, String setting) {
    if (value < 0 || value > 255) {
      state = State.INVALID;
      System.out.println(
          new StringBuilder("Color values must be between 0 and 255 but the value for ")
              .append(setting)
              .append(" is: ")
              .append(value)
              .append('.')
  ...
  /**
   * Constructs an instance that fills the panel with black, that is, turns the panel completely
   * off. Users should invoke setters to configure the message.
   */
  ...
  public DisplayMessage setDelay1(int delay) {
    if (delay < 0) {
      System.out.println("Delays must be non-negative but delay 1 is " + delay + '.');
      state = State.INVALID;
    }
    delay1 = delay;
    return this;
  ...
  public DisplayMessage setDelay2(int delay) {
    if (delay < 0) {
      state = State.INVALID;
      System.out.println("Delays must be non-negative but delay 2 is " + delay + '.');
    }
    delay2 = delay;
    return this;
  }
  ...
  public String toString() {
    ...
    switch (state) {
      case VALID:
        result =
            new StringBuilder(displayText.isEmpty() ? " " : displayText)
                .append('|')
  ...
  public byte[] toByteArray() {
    ...
    byte[] serializedCommand = toString().getBytes(StandardCharsets.US_ASCII);
    System.arraycopy(serializedCommand, 0, result, 0, serializedCommand.length);
