...
public class SpiDisplayConnection implements DisplayConnection {
  ...
  /**
   * {@link DisplayConnection} implementation that sends message over an <a
   * href='https://first.wpi.edu/wpilib/allwpilib/docs/release/java/edu/wpi/first/wpilibj/SPI.html'>SPI</a>
   * link
   *
   * @param port SPI port, the RoboRio SPI pin to connect to the ESP32's Slave Select GPIO pin.
   */
  ...
  public int send(DisplayMessage message) {
    ...
    if (messageSize != 128) {
      throw new IllegalStateException("Message send length should be 128, is " + messageSize);
    }
