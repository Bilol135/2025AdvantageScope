...
public class DisplayConnectionFactory {
  /**
   * Connect to the Marquee over a serial USB cable. For the connection to succeed, there must be
   * one and only one USB serial device connected to the RoboRio. Note that serial devices have
   * paths of the form {@code /dev/ttyUSBn}, where {@code n} is 0, 1, or 2.
   *
   * @return a {@link UsbDisplayConnection} if it was possible to connect, a {@link
   *     VacuousDisplayConnection} otherwise.
   */
   ...
   *
   * @param usbDevicePaths a {@link List} of available serial USB devices as {@link Path} instances.
   * @return a {@link DeviceConnection} implementation. If {usbDevicePaths} contains a single {@link
   *     Path} and connection succeeds, then the invocation will return a {@link
   *     UsbDisplayConnection}. Otherwise it will return a {@link VacuousDisplayConnection}.
   */
  private static DisplayConnection connectionFromUsbPaths(List<Path> usbDevicePaths) {
    ...
    switch (usbDevicePaths.size()) {
      ...
      case 1:
        ...
        connection = UsbDisplayConnection.builder(usbDevicePaths.get(0)).build();
        break;
      default:
