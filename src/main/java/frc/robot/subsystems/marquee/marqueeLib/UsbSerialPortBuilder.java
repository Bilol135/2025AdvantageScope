...
/**
 * Builds a {@link SerialPort} from its file path and communication settings. The default
 * communication settings are 115200 bits/second, 8 data bits, one stop bit, no parity, no flow
 * control, and flush on write, The default values support the marquee, but can be changed to
...
public class UsbSerialPortBuilder {
  ...
  static {
    ...
    USB_PATH_TO_SERIAL_PORT.put(defaultFileSystem.getPath("/dev", "ttyUSB0"), SerialPort.Port.kUSB);
  ...
  /**
   * Creates a {@code UsbSerialPathBuilder} for the specified device path and default communication
   * settings
   *
   * @param devicePath the serial USB device {@link Path}. See {@code USB_PATH_TO_SERIAL_PORT} for a
   *     list of supported device paths.
   */
   ...
   *
   * @return the newly created and configured {@link SerialPort} if the settings valud, {@code null}
   *     otherwise
   */
  public SerialPort build() {
    ...
    if (devicePath != null) {
      ...
      if (usbPort != null) {
        built = new SerialPort(baudRate, usbPort, dataBits, parity, stopBits);
