...
/** A {@link DeviceConnection} that sends marquee commands over a USB serial connection. */
public class UsbDisplayConnection implements DisplayConnection {
  ...
  /**
   * Sends the specified {@code message} to the ESP32 over a serial USB connection.
   *
   * @param message message to send
   * @return the number of bytes sent, which should be in the range [0 .. 127]
   */
  @Override
  public int send(DisplayMessage message) {
    try {
      ...
      System.out.println("Exception while sending message to the Marquee.\n" + e);
      return 0;
    }
  }

  /**
   * Returns a {@link Builder} that will create a {@link UsbDisplayConnection} that sends display
   * commands to a specified serial USB device.
   *
